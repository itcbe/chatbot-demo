...
def recommendation():
    ...
    with open('cafes.csv', 'rt') as csvfile:
...
@route('/', method='GET')
def index():
    return template("templates/chatbot.html")


@route("/chat", method='POST')
def chat():

    user_msg = request.forms.get('msg').lower()
    ...
    response_json = json.dumps({"animation": animation, "msg": response_msg})
    response.set_header('Access-Control-Allow-Origin', '*')

    return response_json


@route('/static/js/<filename:re:.*\.js>', method='GET')
